...
def _load_feature(batch):
    ...
    Returns:
        (keys, feats, labels_ctc, labels_att)
    """
    keys = []
    feats = []
    lengths = []
    ...
    labels = [np.fromiter(map(int, x), dtype=np.int32) for x in labels]
    sorted_labels = [labels[i] for i in order]
    labels_att = [x[3].split() for x in batch]
    labels_att = [np.fromiter(map(int, x), dtype=np.int32) for x in labels_att]
    sorted_labels_att = [labels_att[i] for i in order]
    return sorted_keys, sorted_feats, sorted_labels, sorted_labels_att
...
class CollateFunc(object):
    ...
    def __call__(self, batch):
        ...
        if self.raw_wav:
            ...
            keys, xs, ys, ys_att = _load_feature(batch[0])
        ...
        if train_flag:
            ...
            ys_att_lengths = torch.from_numpy(
                np.array([y.shape[0] for y in ys_att], dtype=np.int32))
            if len(ys) > 0:
                ys_pad = pad_sequence([torch.from_numpy(y).int() for y in ys],
                                      True, IGNORE_ID)
                ys_att_pad = pad_sequence([torch.from_numpy(y).int() for y in ys_att],
                                          True, IGNORE_ID)
            else:
                ys_pad = torch.Tensor(ys)
                ys_att_pad = torch.Tensor(ys_att)
        else:
            ys_pad = None
            ys_lengths = None
            ys_att_lengths = None
            ys_att_pad = None
        return keys, xs_pad, ys_pad, xs_lengths, ys_lengths, ys_att_pad, ys_att_lengths
    ...
    def __init__(self,
        ...
        with codecs.open(data_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                tokenid_att = arr[8].split(':')[1]
                output_dim_att = int(arr[9].split(':')[1].split(',')[1])
                if raw_wav:
                    ...
                    data.append((key, feat_ark, num_frames, tokenid, tokenid_att))
                    self.input_dim = feat_dim
                self.output_dim = output_dim
                self.output_dim_att = output_dim_att
        if sort:
        ...
        if batch_type == 'dynamic':
            ...
            for i in range(num_data):
                ...
                if num_frames_in_batch > max_frames_in_batch:
                    self.minibatch.append([])
                    num_frames_in_batch = length
                self.minibatch[-1].append((data[i][0], data[i][1], data[i][3], data[i][4]))
        # Static batch size
        else:
            cur = 0
            while cur < num_data:
...
if __name__ == '__main__':
    ...
    for i, batch in enumerate(data_loader):
        print(i, len(batch), batch[1].shape)
        # print(batch[1].shape)

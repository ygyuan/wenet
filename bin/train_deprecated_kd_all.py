...
from wenet.kd.asr_model_kd_all import init_asr_model_kd
from wenet.utils.checkpoint import (load_checkpoint_kd, save_checkpoint,
                                    load_trained_modules)
from wenet.kd.executor import Executor
...
the future, please move to the new IO !!!
    ...
    model = init_asr_model_kd(configs)
    print(model)
    num_params = sum(p.numel() for p in model.parameters())
    print('the number of model params: {:.2f}M'.format(num_params/1048576.))
    ...
    if args.checkpoint is not None:
        infos = load_checkpoint_kd(model, args.checkpoint)

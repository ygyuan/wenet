        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            with context():
                ...
                with torch.cuda.amp.autocast(scaler is not None):
                    loss, loss_att, loss_ctc, loss_ctc_mid = model(feats, feats_lengths,
            ...
            if batch_idx % log_interval == 0:
                ...
                if loss_ctc_mid is not None:
                    log_str += 'loss_ctc_mid {:.6f} '.format(loss_ctc_mid.item())
    ...
    def cv(self, model, data_loader, device, args):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(data_loader):
                ...
                if num_utts == 0:
                    continue
                loss, loss_att, loss_ctc, loss_ctc_mid = model(feats, feats_lengths, target,
                                                               target_lengths)
                #print("cv: ", loss, loss_att, loss_ctc, loss_ctc_mid)
                if torch.isfinite(loss):
                    ...
                    if batch_idx % log_interval == 0:
                        ...
                        log_str += 'history loss {:.6f}'.format(total_loss /
                                                                num_seen_utts)
                        logging.debug(log_str)
            print("total loss {:.6f} num_see_utts {:d}".format(total_loss, num_seen_utts))

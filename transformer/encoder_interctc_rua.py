...
from wenet.transformer.encoder_layer_rua import TransformerEncoderLayer
from wenet.transformer.encoder_layer_rua import ConformerEncoderLayer
from wenet.transformer.encoder_layer_rua import NullMHAConformerEncoderLayer
...
class BaseEncoder(torch.nn.Module):
    ...
    def forward(
        ...
        """
        T = xs.size(1)
        #print(xs_lens.size(), T)
        masks = ~make_pad_mask(xs_lens, T).unsqueeze(1)  # (B, 1, T)
        #masks = ~make_pad_mask(xs_lens).unsqueeze(1)  # (B, 1, T)
        #print(xs_lens.max(), xs.size(1), masks.size())
        ...
        ctc_layer_index = [2, 5, 8, 11]
        inputs = xs

        new_att_score = torch.tensor([])                                        
        new_cnn_cache = torch.tensor([])
        for i, layer in enumerate(self.encoders):
            #inputs, chunk_masks, _, _ = layer(inputs, chunk_masks, pos_emb, mask_pad)
            if i ==0:
                inputs, chunk_masks, new_att_score, _ = layer(inputs, chunk_masks, pos_emb, mask_pad)
            else:
                inputs, chunk_masks, new_att_score, _ = layer(inputs, chunk_masks, new_att_score, mask_pad)

            #inputs, chunk_masks, _ = layer(inputs, chunk_masks, pos_emb, mask_pad)
            #print(inputs.size(), chunk_masks.size())

            if i in ctc_layer_index:
                if self.normalize_before:
                    inputs = self.after_norm(inputs)
                if i == 2:
                    xs = inputs.unsqueeze(0)
                else:
                    xs = torch.cat((xs, inputs.unsqueeze(0)))
            #inputs = outputs

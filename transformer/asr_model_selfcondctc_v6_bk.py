...
from wenet.transformer.encoder_selfcondctc_v6 import ConformerEncoder
from wenet.transformer.encoder_selfcondctc_v6 import TransformerEncoder
...
class ASRModel(torch.nn.Module):
    ...
    def forward(
        ...
        # 1. Encoder
        encoder_out, intermediate_outs, encoder_mask = self.encoder(speech, speech_lengths)
        ...
        if self.ctc_weight != 0.0:
            loss_ctc = self.ctc(encoder_out, encoder_out_lens, text,
                                text_lengths)
            for i in range(intermediate_outs.size(0)):
                loss_ctc += self.ctc(intermediate_outs[i], encoder_out_lens, text, text_lengths) / intermediate_outs.size(0)
            loss_ctc = 0.5 * loss_ctc
        else:
            loss_ctc = None
    ...
    def _forward_encoder(
        ...
        if simulate_streaming and decoding_chunk_size > 0:
            ...
            encoder_out, intermediate_outs, encoder_mask = self.encoder(
                speech,
                speech_lengths,
                decoding_chunk_size=decoding_chunk_size,
                num_decoding_left_chunks=num_decoding_left_chunks

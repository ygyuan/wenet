...
from wenet.transformer.ctc_branch import CTCBranch
from wenet.transformer.encoder_onnx import ConformerEncoder
from wenet.transformer.encoder_onnx import TransformerEncoder
...
class ASRModel(torch.nn.Module):
    ...
    def forward(
        ...
        text_att: torch.Tensor,
        text_att_lengths: torch.Tensor,
        ...
        assert text_att_lengths.dim() == 1, text_att_lengths.shape
        ...
        if self.ctc_weight != 0.0:
            loss_ctc = self.ctc(encoder_out, encoder_out_lens, text,
                                text_lengths)
            loss_ctc_char = self.ctc_char(encoder_out, encoder_out_lens, text_att, text_att_lengths)
        else:
            loss_ctc = None
            loss_ctc_char = None
        ...
        if loss_ctc is None:
            ...
            loss = loss_ctc + loss_ctc_char
        else:
            loss = self.ctc_weight * (loss_ctc + loss_ctc_char ) + (1 - self.ctc_weight) * loss_att
        return loss, loss_att, loss_ctc, loss_ctc_char
    ...
    def ctc_greedy_search(
        ...
        #print("ctc_probs: ", ctc_probs.size())
        ctc_dim = ctc_probs.size(2)
        topk_prob, topk_index = ctc_probs.topk(1, dim=2)  # (B, maxlen, 1)
        topk_index = topk_index.view(batch_size, maxlen)  # (B, maxlen)
        mask = make_pad_mask(encoder_out_lens)  # (B, maxlen)
        #print(mask)
        topk_index = topk_index.masked_fill_(mask, ctc_dim-1)  # (B, maxlen)
        #print(topk_index)
...
def init_asr_model(configs):
    ...
    vocab_size_att = configs['output_dim_att']
    ...
    if decoder_type == 'transformer':
        decoder = TransformerDecoder(vocab_size_att, encoder.output_size(),
                                     **configs['decoder_conf'])
    else:
        assert 0.0 < configs['model_conf']['reverse_weight'] < 1.0
        assert configs['decoder_conf']['r_num_blocks'] > 0
        decoder = BiTransformerDecoder(vocab_size_att, encoder.output_size(),
                                       **configs['decoder_conf'])
    ctc = CTC(vocab_size, encoder.output_size())
    ctc_char = CTC(vocab_size_att, encoder.output_size())
    model = ASRModel(
        vocab_size=vocab_size_att,
        encoder=encoder,
        decoder=decoder,
        ctc=ctc,
        ctc_char=ctc_char,
...
class ASRCTCBranchCMVN(torch.nn.Module):
    """CTC-attention hybrid Encoder-Decoder model"""
    def __init__(
        self,
        vocab_size: int,

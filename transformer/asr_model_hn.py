...
from wenet.transformer.encoder_hn import ConformerEncoder
from wenet.transformer.encoder_hn import TransformerEncoder
...
class ASRModel(torch.nn.Module):
    ...
    def forward(
        ...
        # 1. Encoder
        encoder_out_mid, encoder_out, encoder_mask = self.encoder(speech, speech_lengths)
        ...
        if self.ctc_weight != 1.0:
            loss_att, acc_att = self._calc_att_loss(encoder_out, encoder_mask,
                                                    text, text_lengths)
            loss_att_mid, acc_att_mid = self._calc_att_loss(encoder_out_mid, encoder_mask, text, text_lengths)
        else:
            loss_att = None
            loss_att_mid = None
        ...
        if self.ctc_weight != 0.0:
            ...
            loss_ctc_mid = self.ctc(encoder_out_mid, encoder_out_lens, text, text_lengths)
        else:
            loss_ctc = None
            loss_ctc_mid = None


        #print(loss_ctc_mid, loss_ctc)
        if loss_ctc is None or loss_ctc_mid is None:
            loss = loss_att
        elif loss_att is None or loss_att_mid is None:
            loss = loss_ctc + loss_ctc_mid
        else:
            #loss = self.ctc_weight * (loss_ctc) + (1 - self.ctc_weight) * loss_att
            #loss = self.ctc_weight * (loss_ctc + loss_ctc_mid) + (1 - self.ctc_weight) * loss_att
            loss = self.ctc_weight * (loss_ctc + loss_ctc_mid) + (1 - self.ctc_weight) * (loss_att + loss_att_mid)
        return loss, loss_att, loss_ctc, loss_att_mid
    ...
    def _forward_encoder(
        ...
        if simulate_streaming and decoding_chunk_size > 0:
            encoder_out_mid, encoder_out, encoder_mask = self.encoder.forward_chunk_by_chunk(
                speech,
                decoding_chunk_size=decoding_chunk_size,
                num_decoding_left_chunks=num_decoding_left_chunks
            )  # (B, maxlen, encoder_dim)
        else:
            encoder_out_mid, encoder_out, encoder_mask = self.encoder(
                speech,
                speech_lengths,
                decoding_chunk_size=decoding_chunk_size,
                num_decoding_left_chunks=num_decoding_left_chunks
            )  # (B, maxlen, encoder_dim)
        return encoder_out_mid, encoder_out, encoder_mask
    ...
    def recognize(
        ...
        # 1. Encoder
        encoder_out_mid, encoder_out, encoder_mask = self._forward_encoder(
    ...
    def mid_ctc_greedy_search(
        ...
        encoder_out_mid, encoder_out, encoder_mask = self._forward_encoder(
            ...
            simulate_streaming)  # (B, maxlen, encoder_dim)
        #maxlen = encoder_out.size(1)
        maxlen = encoder_out_mid.size(1)
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)
        #ctc_probs = self.ctc.log_softmax(
        #    encoder_out)  # (B, maxlen, vocab_size)
        ctc_probs = self.ctc.log_softmax(encoder_out_mid) # (B, maxlen, vocab_size)
        topk_prob, topk_index = ctc_probs.topk(1, dim=2)  # (B, maxlen, 1)
        topk_index = topk_index.view(batch_size, maxlen)  # (B, maxlen)
        mask = make_pad_mask(encoder_out_lens)  # (B, maxlen)
        topk_index = topk_index.masked_fill_(mask, self.eos)  # (B, maxlen)
    ...
    def forward_encoder_chunk_batch(                                            
        ...
        """                                                                     
        #print(xs, xs_len)                                                      
        encoder_out_mid, encoder_out, encoder_mask = self.encoder.forward(xs, xs_len)                
        #print(encoder_out, encoder_mask)                                           
        return encoder_out_mid, encoder_out, encoder_mask

    #@torch.jit.export
    #def forward_encoder_first_chunk_batch(
    #    self,
    #    xs: torch.Tensor,
    #    xs_len: torch.Tensor,
    #) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    #    """
    #    #print(xs, xs_len) 
    #    encoder, pos_emb, masks = self.encoder.forward_first(xs, xs_len)
    #    #print(encoder_out_mid, encoder, encoder_mask)                                          
    #    return encoder, pos_emb, masks

    #@torch.jit.export
    #def forward_encoder_second_chunk_batch(
    #    self,
    #    xs: torch.Tensor,
    ...
    #                                                                            
    #    """
    #    encoder, pos_emb, masks = self.encoder.forward_second(xs, pos_emb, masks)
    #    return encoder, pos_emb, masks
    ...
    #                                                                            
    #    """
    #    encoder, masks = self.encoder.forward_all(xs, xs_len)
    #    return encoder, masks

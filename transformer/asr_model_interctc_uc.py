...
class ASRModel(torch.nn.Module):
    ...
    def ctc_greedy_search(
        ...
        for i in range(encoder_out.size(0)):
            maxlen = encoder_out[i].size(1)
            encoder_out_lens = encoder_mask.squeeze(1).sum(1)
            #ctc_probs = self.ctc.log_softmax(
            #    encoder_out[i])  # (B, maxlen, vocab_size)
            ctc_softmax = self.ctc.softmax(encoder_out[i])
            ctc_probs = torch.log(ctc_softmax)
            ctc_uncertainty = (ctc_softmax*ctc_probs).sum(2)/torch.log(torch.tensor(1.0/self.vocab_size))
            ctc_uncertainty2 = torch.mean(ctc_uncertainty, 1)
            ...
            uncertainty_list.append(ctc_uncertainty)
            print(i, ctc_uncertainty2, hyps, ctc_uncertainty)
        return hyps, scores

...
from wenet.transformer.attention_rua_onnx import MultiHeadedAttention
from wenet.transformer.attention_rua_onnx import RelPositionMultiHeadedAttention
from wenet.transformer.attention_rua_onnx import NullMultiHeadedAttention
from wenet.transformer.attention_rua_onnx import NullRelPositionMultiHeadedAttention
from wenet.transformer.convolution import ConvolutionModule
from wenet.transformer.embedding_onnx import PositionalEncoding
from wenet.transformer.embedding_onnx import RelPositionalEncoding
from wenet.transformer.embedding_onnx import NoPositionalEncoding
from wenet.transformer.encoder_layer_rua_onnx import TransformerEncoderLayer
from wenet.transformer.encoder_layer_rua_onnx import ConformerEncoderLayer
from wenet.transformer.encoder_layer_rua_onnx import NullMHAConformerEncoderLayer
...
class BaseEncoder(torch.nn.Module):
    ...
    def forward(
        ...
        #mid_xs = self.after_norm(mid_xs)
        # Here we assume the mask is not changed in encoder layers, so just
        # return the masks before encoder layers, and the masks will be used
        # for cross attention with decoder later
        # print(mid_xs.size(), xs.size(), masks.size())
        #print(xs.size(), xs)
        #return mid_xs, xs, masks
        return xs
    ...
    def forward_chunk(
        ...
        return (mid_xs[:, cache_size:, :], xs[:, cache_size:, :], r_subsampling_cache,
    ...
    def forward_chunk_by_chunk(
        ...
        for cur in range(0, num_frames - context + 1, stride):
            ...
            chunk_xs = xs[:, cur:end, :]
            (mid_y, y, subsampling_cache, elayers_output_cache,
             conformer_cnn_cache) = self.forward_chunk(chunk_xs, offset,
                                                       ...
                                                       conformer_cnn_cache)
            outputs.append(y)
            mid_outputs.append(mid_y)
            offset += y.size(1)
        ys = torch.cat(outputs, 1)
        mid_ys = torch.cat(mid_outputs, 1)
        masks = torch.ones(1, ys.size(1), device=ys.device, dtype=torch.bool)
        masks = masks.unsqueeze(1)
        return mid_ys, ys, masks

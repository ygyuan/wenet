...
class ConformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        x_att, x_attscore = self.self_attn(x_q, x, x, mask, pos_emb)
        if self.concat_after:
        ...
        return x, mask, x_attscore, new_cnn_cache
...
class NullMHAConformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if output_cache is not None:
            x = torch.cat([output_cache, x], dim=1)
        #print(x.size(), mask.size(), new_cnn_cache.size())
        return x, mask, att_scores, new_cnn_cache
